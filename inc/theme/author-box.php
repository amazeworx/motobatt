 ...
 *
 */
function gfs_author_archive_avatar( $title, $description, $context ) {
	if( ! is_author() || get_query_var( 'paged' ) )
		return;

	echo get_avatar( get_queried_object_id(), 200 );
}
add_filter( 'genesis_archive_title_descriptions', 'gfs_author_archive_avatar', 9, 3 );

/**
 * Author archive introduction
 *
 */
function gfs_author_intro( $title, $description, $context ) {
	if( ! is_author() || get_query_var( 'paged' ) )
		return;

	$title = get_user_meta( get_queried_object_id(), 'gfs_user_title', true );
	if( !empty( $title ) )
		echo '<p class="author-title">' . esc_html( $title ) . '</p>';

	if( empty( $description ) )
		echo apply_filters( 'gfs_the_content', get_the_author_meta( 'description' ) );

	echo '<h3 class="has-text-align-center">Recent Articles by ' . get_the_author_meta( 'first_name' ) . '</h3>';
}
add_action( 'genesis_archive_title_descriptions', 'gfs_author_intro', 11, 3 );
